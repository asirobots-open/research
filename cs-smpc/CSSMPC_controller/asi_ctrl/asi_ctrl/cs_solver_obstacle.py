...
class CSSolver:
    def __init__(self, n, m, l, N, v_range, slew_rate, obstacles, mean_only=False, k_form=0, prob_lvl=0.95, chance_const_N=-1, boundary_dim=-2, delta_slew_cost=1.0):
        try:
            ...
            if obstacles[0]:
                ...
                for ii in range(N):
                    x_matrix[ii, ii * n + n-2] = 1
                y_matrix = np.zeros((N, n * N))
                for ii in range(N):
                    y_matrix[ii, ii * n + n-1] = 1
            ...
            # chance constraint
            beta = M.parameter([2, N])
            for ii in range(chance_const_N):
                alpha = np.zeros((n, 1))
                alpha[boundary_dim, 0] = 1
                ...
                if not mean_only:
                    ...
                    M.constraint(Expr.sub(Expr.add(mean_part, Expr.mul(cov_part, inv_prob)), beta.slice([0, 0], [1, N])),
                                 Domain.lessThan(0))
                    M.constraint(Expr.add(Expr.sub(mean_part, Expr.mul(cov_part, inv_prob)), beta.slice([1, 0], [2, N])),
                                 Domain.greaterThan(0))
                else:
                    cov_part = covariance_chance_constraints.index([ii, 0])
                    M.constraint(Expr.sub(Expr.add(mean_part, Expr.mul(cov_part, Expr.constTerm(inv_prob))), beta.slice([0, ii], [1, ii+1])), Domain.lessThan(0))
                    M.constraint(Expr.add(Expr.sub(mean_part, Expr.mul(cov_part, Expr.constTerm(inv_prob))), beta.slice([1, ii], [2, ii+1])),
                                 Domain.greaterThan(0))
    ...
    def populate_params(self, A, B, d, D, mu_0, sigma_0, sigma_N_inv, Q_bar, R_bar, u_0, x_target, mu_N, track_width, K=None, obs=None):
        ...
        if isinstance(track_width, np.ndarray):
            ...
            self.beta.setValue(np.tile(track_width, (2, N)) / 2)
        ...
        if self.mean_only:
            ...
            D_part_Ek_alpha = D_part_Ek[:, -2:, :]
            sigma0_part_Ek_alpha = sigma0_part_Ek[:, -2:, :]
